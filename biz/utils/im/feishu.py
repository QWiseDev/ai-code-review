...
class FeishuNotifier:
    ...
    def send_message(self, content, msg_type='text', title=None, is_at_all=False, project_name=None, url_slug=None):
        ...
        try:
            ...
            if project_config:
                if not project_config.get('feishu_enabled'):
                    logger.info(f"项目 {identifier} 未启用飞书推送，跳过发送。")
                    return
                post_url = (project_config.get('feishu_webhook_url') or '').strip()
                if not post_url:
                    logger.warning(f"项目 {identifier} 已启用飞书推送但未配置 webhook 地址，跳过发送。")
                    return
            else:
